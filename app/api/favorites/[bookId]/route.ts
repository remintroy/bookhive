...
export async function POST(req: NextRequest, { params }: { params: Promise<{ bookId?: string }> }) {
  const requestHandler = async (user: UserRecord | null) => {
    try {
      const bookId = (await params)?.bookId?.trim?.();
      ...
      await Favorites.updateOne(
        { userId: user?.uid, bookId: new mongoose.Types.ObjectId(bookId) }, // Find condition
        { $setOnInsert: { userId: user?.uid, bookId: new mongoose.Types.ObjectId(bookId) } }, // Set only on insert
        { upsert: true }
      );
